...
void print_meta(const MOBIData *m) {
    ...
    if (m->mh) {
        ...
        if (m->mh->huff_rec_index) {
            printf("huffman record offset: %u\n", *m->mh->huff_rec_index);
        }
        if (m->mh->huff_rec_count) {
            printf("huffman records count: %u\n", *m->mh->huff_rec_count);
        }
 ...
 */
static int
create_path(char *newpath, const size_t buf_len, const char *fullpath, const char *suffix) {
 ...
 */
static int
create_dir(char *newdir, const size_t buf_len, const char *fullpath, const char *suffix) {
...
static int dump_rawml_parts(const MOBIRawml *rawml, const char *fullpath) {
    ...
    if (create_epub_opt) {
        ...
        if (write_to_dir(opfdir, "container.xml", (const unsigned char *) EPUB_CONTAINER,
                         sizeof(EPUB_CONTAINER) - 1) == ERROR) {
            return ERROR;
        }

        /* create mimetype file */
        if (write_to_dir(opfdir, "mimetype", (const unsigned char *) EPUB_MIMETYPE,
                         sizeof(EPUB_MIMETYPE) - 1) == ERROR) {
            return ERROR;
        }
        ...
        while (curr != NULL) {
            ...
            if (curr->size > 0) {
                ...
                if (create_epub_opt && file_meta.type == T_OPF) {
                    ...
                    n = snprintf(partname, sizeof(partname), "%s%cresource%05zu.%s", newdir,
                                 separator, curr->uid, file_meta.extension);
                }
                if (n < 0) {
                    printf("Creating file name failed\n");
                    return ERROR;
...
int main(int argc, char *argv[]) {
    ...
    while ((c = getopt(argc, argv,
                       "cd" PRINT_EPUB_ARG "himo:" PRINT_ENC_ARG "rst" PRINT_RUSAGE_ARG "vx7")) !=
           -1) {
        switch (c) {
            case 'c':
                dump_cover_opt = true;
