...
jobject get_djvu_hyperlink_mapping(JNIEnv *jenv, ddjvu_document_t *djvu_document,
    ...
    if (miniexp_caar(iter) == miniexp_symbol("url")) {
        if (!string_from_miniexp(miniexp_cadr(miniexp_car(iter)), &url)) {
            DEBUG_PRINT("DjvuLibre error: Unknown hyperlink %s",
    ...
    jclass pagelinkClass = jenv->FindClass("com/xiaoyv/comic/datasource/jni/codec/PageLink");
    if (!pagelinkClass) {
        delete[] data;
        return hl;
    }
...
Java_com_xiaoyv_comic_datasource_impl_djvu_DjvuBookPage_renderPage(JNIEnv *env, jclass cls,
    ...
    targetRect.x = pageSliceX;
    targetRect.y = pageSliceY;
    targetRect.w = pageSliceWidth;
    targetRect.h = pageSliceHeight;
    ...
    jboolean result = ddjvu_page_render(page, (ddjvu_render_mode_t) rendermode, &pageRect,
                                        &targetRect, pixelFormat,
